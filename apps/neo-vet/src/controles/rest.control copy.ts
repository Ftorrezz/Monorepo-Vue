...
class NdPeticionControl {
  ...
  public async invocarMetodo(nombreMetodo: string, verbo: string, modelo?: any, parametros?: DtoParametros, atributos?: string[]) {
    ...
    const idsitio = store.id_sitio;
    const idsucursal = store.id_sucursal;
  ...
  public procesarRespuesta(respuesta: any, _urlEndPoint: string): any {
    // Si no hay datos en la respuesta, devolvemos un objeto de resultado vacío
    if (!respuesta || !respuesta.data) return { result: [] };

    const data = respuesta.data;
    const alertas = new NdAlertasControl();

    // Caso 1: La respuesta sigue el patrón ND (Array con objeto de control: [{ errores, mensajes, elemento }])
    if (Array.isArray(data) && data.length === 1 && (data[0].errores || data[0].mensajes || data[0].elemento !== undefined)) {
      const wrapped = data[0];

      if (Array.isArray(wrapped.errores)) {
        for (let i = 0; i < wrapped.errores.length; i++) {
          alertas.mostrarMensaje('error', 'Error', wrapped.errores[i].mensaje);
        }
      }

      if (Array.isArray(wrapped.mensajes)) {
        for (let i = 0; i < wrapped.mensajes.length; i++) {
          alertas.mostrarMensaje(wrapped.mensajes[i].tipomensaje, 'Verificación', wrapped.mensajes[i].mensaje);
        }
      }

      return wrapped.elemento !== undefined ? wrapped.elemento : wrapped;
    }

    // Caso 2: La respuesta es un array de datos directo (ej. lo reportado por el usuario) o un objeto simple
    return data;
  }
