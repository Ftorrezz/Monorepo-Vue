...
export function useAgenda() {
    ...
    const formatDateKey = (date) => {
        ...
        // 2. Lógica definitiva para distinguir fechas locales vs UTC
        // Si la hora local es 0, es una fecha creada en el frontend (local midnight)
        if (d.getHours() === 0 && d.getMinutes() === 0) {
            ...
            // En cualquier otro caso (incluyendo backend UTC midnight), usar componentes UTC
            // Esto es seguro porque las fechas del backend suelen ser UTC midnight
            // y las fechas con hora (citas) también funcionan bien con sus componentes UTC para la clave del día
    ...
    const formatTime = (timeStr) => {
        ...
        if (timeStr.includes('T')) {
            ...
            // Usar horas y minutos locales para que coincidan con la zona horaria del usuario
...
const calendarDays = computed(() => {
    ...
    const now = new Date()
    // Normalizar "hoy" al inicio del día para comparaciones precisas
...
const updateSelectedDate = (newDate) => {
    // Si newDate es YYYY-MM-DD, añadir T00:00:00 para forzar el parseo local en algunos navegadores
...
const onCitaCreada = (cita) => {
    ...
    // Limpiar cache del día actual para forzar recarga
    const dateKey = formatDateKey(selectedDate.value)
    const cacheKey = `${selectedService.value.id}-${dateKey}`
