          ...
          >
            ...
            <div class="service-lead">
              <div class="service-icon-wrapper" :class="servicio.completado ? 'bg-positive-soft' : 'bg-orange-soft'">
                <q-icon :name="servicio.icono" :color="servicio.completado ? 'positive' : 'orange'" size="28px" />
              </div>
                ...
                <div class="service-action-group">
                  ...
                  <div class="service-quick-actions-header" v-if="servicio.completado">
                    <q-btn
                      ...
                      flat round dense icon="card_membership" size="10.5px" color="primary" class="action-btn-v2"
                    ...
                    <q-btn
                      ...
                      flat round dense icon="description" size="10.5px" color="primary" class="action-btn-v2"
                      @click.stop="$emit('imprimir-servicio', servicio, 'especial')"
                    ><q-tooltip>Imprimir Consulta</q-tooltip></q-btn>

                    <q-btn
                      v-if="servicio.tipo?.toLowerCase() === 'consulta' || servicio.componente_clave === 'consulta' || servicio.tipo?.toLowerCase() === 'vacunacion' || servicio.componente_clave === 'vacunacion' || servicio.tipo?.toLowerCase() === 'receta' || servicio.componente_clave === 'receta' || servicio.tipo?.toLowerCase() === 'desparasitacion' || servicio.componente_clave === 'desparasitacion'"
                      flat round dense icon="history_edu" size="10.5px" color="orange-8" class="action-btn-v2"
                      @click.stop="$emit('firmar-servicio', servicio, 'especial')"
                    ><q-tooltip>Visualizar y Firmar</q-tooltip></q-btn>
                    
                    <q-btn
                      v-if="servicio.tipo?.toLowerCase() === 'receta' || servicio.componente_clave === 'receta'"
                      flat round dense icon="receipt_long" size="10.5px" color="secondary" class="action-btn-v2"
                      @click.stop="$emit('imprimir-servicio', servicio, 'especial')"
                        ...
                        >
                          ...
                          <q-item-section side>
                            <div class="row items-center q-gutter-xs">
                              <q-btn flat round dense icon="print" size="xs" color="grey-7" @click.stop="$emit('imprimir-servicio', servicio, 'plantilla', p.id_plantilla)" />
                              <q-btn flat round dense icon="history_edu" size="xs" color="orange-8" @click.stop="$emit('firmar-servicio', servicio, 'plantilla', p.id_plantilla)" />
                            </div>
                          </q-item-section>
                        </q-item>
                      </q-list>
                    </q-btn-dropdown>
                    <div v-else-if="servicio.plantillas_servicio?.length === 1 || servicio.id_plantilla" class="row items-center no-wrap">
                      <q-btn
                        flat round dense icon="print" size="10.5px" color="grey-7" class="action-btn-v2"
                        @click.stop="$emit('imprimir-servicio', servicio, 'plantilla')"
                      ><q-tooltip>Imprimir</q-tooltip></q-btn>
                      <q-btn
                        flat round dense icon="history_edu" size="10.5px" color="orange-8" class="action-btn-v2"
                        @click.stop="$emit('firmar-servicio', servicio, 'plantilla')"
                      ><q-tooltip>Visualizar y Firmar</q-tooltip></q-btn>
                    </div>
                  </div>
                  ...
                  <q-btn unelevated round dense icon="arrow_forward" size="11px" color="primary" class="go-to-btn" @click="$emit('seleccionar-pestaña', servicio.id)">
