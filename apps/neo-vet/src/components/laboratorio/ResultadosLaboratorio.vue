            ...
            <q-td key="resultado" :props="props" class="resultado-cell">
              <div class="resultado-container">
                <q-input
                  ...
                  @input="(val) => onResultadoInput(props.row, val)"
                  @blur="() => procesarResultado(props.row)"
                ...
                <div v-if="props.row.resultado && props.row.estado !== 'validado'" class="validacion-rapida">
                  <q-btn
                    dense
                    flat
                    round
    ...
    if (Array.isArray(estudio.pruebas)) {
      estudio.pruebas.forEach(prueba => {
        pruebas.push({
          ...
          // Asegurar que resultado tenga un valor inicial
...
const pruebasFiltradasYOrdenadas = computed(() => {
  return pruebasAplanadas.value.sort((a, b) => {
    ...
    // Finalmente por nombre de prueba
...
// Métodos corregidos para manejo de resultados
const onResultadoInput = (prueba: Prueba, event: any) => {
  // Este método se ejecuta en tiempo real mientras se escribe
  const valor = event.target ? event.target.value : event
  
  // Actualizar el valor directamente
  prueba.resultado = valor
  
  console.log(`Input en tiempo real - Prueba: ${prueba.nombre}, Valor: "${valor}"`)
}

const procesarResultado = (prueba: Prueba) => {
  // Este método se ejecuta cuando el input pierde el foco
  console.log(`Procesando resultado - Prueba: ${prueba.nombre}, Valor: "${prueba.resultado}"`)
  
  const valor = prueba.resultado?.toString().trim() || ''
  
  if (!valor) {
    // Si está vacío, marcar como pendiente
    prueba.estado = 'pendiente'
    prueba.usuarioCargo = undefined
    prueba.fechaCarga = undefined
    prueba.resultado = null
  } else {
    // Hay un valor, procesarlo según el tipo
    if (prueba.tipo === 'numerico') {
      ...
      if (!isNaN(valorNumerico)) {
        ...
        actualizarEstadoCargado(prueba)
      } else {
        // No es un número válido, mantener como texto pero avisar
        prueba.resultado = valor
        actualizarEstadoCargado(prueba)
      ...
      // Es tipo texto, guardar como está
      prueba.resultado = valor
      actualizarEstadoCargado(prueba)
    }
    
    // Evaluar si está fuera de rango después de procesar
...
const actualizarEstadoCargado = (prueba: Prueba) => {
...
const focusNextResult = async (currentIndex: number) => {
  ...
  // Buscar el siguiente input de resultado disponible
