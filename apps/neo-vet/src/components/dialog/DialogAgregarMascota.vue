            ...
            <template v-slot:before>
              <q-tabs v-model="tab" vertical class="text-teal">
                ...
                <q-tab 
                  ...
                  icon="pets" 
                  label="Mascota" 
                  :disable="!formData.propietario.id_poblador" 
                />
              </q-tabs>
            </template>
            ...
            <template v-slot:after>
              ...
              >
                ...
                <q-tab-panel name="mascota">
                  <div class="text-h5 q-mb-md text-teal">
                    Mascota
                    <span 
                      v-if="formData.propietario.id_poblador && (formData.propietario.nombre || formData.propietario.primerapellido)" 
                      class="text-subtitle1 text-grey-8 q-ml-sm"
                    >
                      (Propietario: {{ formData.propietario.nombre }} {{ formData.propietario.primerapellido }})
                    </span>
                  </div>
                  <q-separator
                    class="q-mb-md"
...
import { ref, onMounted, watch, nextTick } from "vue"; // Asegúrate de importar nextTick si no está
...
onMounted(async () => {
  await cargarListas(); // Asumiendo que esta función ya existe y carga catálogos

  if (props.propietarioId) {
    await cargarDatosPropietarioSiExiste(); // Esta función debe poblar formData.propietario
    // Verificar si el propietario se cargó correctamente antes de cambiar de pestaña
    if (formData.value.propietario.id_poblador) {
      tab.value = "mascota";
    }
  }
  if (props.mascotaId) {
    await cargarDatosMascotaSiExiste(); // Esta función debe poblar formData.mascota
    // Si se carga una mascota, y su propietario también está cargado, asegurar que la pestaña sea mascota
    if (formData.value.mascota.id_mascota && formData.value.propietario.id_poblador) {
        tab.value = "mascota";
    }
  }
});

// Asegúrate de que la función guardarPropietario (o como se llame)
// actualice formData.propietario.id_poblador después de guardar exitosamente.
// Ejemplo conceptual (la implementación real puede variar):
/*
const guardarPropietario = async () => {
  // ... lógica para validar y enviar datos ...
  if (guardadoExitoso) {
    formData.value.propietario.id_poblador = respuestaDelServidor.id_poblador;
    formData.value.propietario.nombre = respuestaDelServidor.nombre; // y otros campos relevantes
    formData.value.propietario.primerapellido = respuestaDelServidor.primerapellido;
    // ...
    emit('propietario-agregado', respuestaDelServidor.id_poblador);
    alertas.mostrarMensaje("Propietario guardado", "positive");
    // Opcionalmente, cambiar a la pestaña de mascota si se desea
    // tab.value = "mascota"; 
  } else {
    alertas.mostrarMensaje("Error al guardar propietario", "negative");
  }
};
*/

// La función cargarDatosPropietarioSiExiste debe asegurarse de que
// formData.propietario (incluyendo id_poblador, nombre, primerapellido) se popule.
// Ejemplo conceptual:
/*
const cargarDatosPropietarioSiExiste = async () => {
  if (props.propietarioId) {
    try {
      // const datos = await servicioParaObtenerPropietario(props.propietarioId);
      // formData.value.propietario = { ...datos }; // Asegúrate que 'datos' tenga id_poblador, nombre, etc.
    } catch (error) {
      console.error("Error al cargar datos del propietario:", error);
      // Manejar error, quizás mostrar alerta
    }
  }
};
*/
