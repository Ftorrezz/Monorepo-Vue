...
export const usePropietarioStore = defineStore('propietarioStore', {
  ...
  actions: {
    ...
    setDatosOriginales(datos: any[]) {
      ...
      if (!this.ultimoPropietarioAgregado) {
        // Si había un propietario seleccionado, intentar mantenerlo
        if (propietarioSeleccionadoActual) {
          const propietarioExiste = datos.some(item => item.id === propietarioSeleccionadoActual);
          ...
          // No había propietario seleccionado, usar selección automática
          this.procesarSeleccionAutomatica();
        }
      }
    },
