const fs = require('fs');
const backupPath = 'c:\\Fuentes\\Desarrollo web\\Neodimio\\Frontend-Neodimio-VueJs\\Monorepo-Vue\\apps\\neo-vet\\src\\pages\\agenda\\Agenda copy 4.vue';
const targetPath = 'c:\\Fuentes\\Desarrollo web\\Neodimio\\Frontend-Neodimio-VueJs\\Monorepo-Vue\\apps\\neo-vet\\src\\pages\\agenda\\Agenda.vue';

let content = fs.readFileSync(backupPath, 'utf8');

// 1. Template Replacement
const oldGridRegex = /<!-- Grid de citas -->[\s\S]*?<div class="appointments-grid">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;
const horizontalTemplate = `<!-- Grid de citas -->
            <div v-if="dayViewMode === 'cards'" class="appointments-grid">
              <div 
...
if (oldGridRegex.test(content)) {
    content = content.replace(oldGridRegex, horizontalTemplate);
    console.log('Template replaced.');
} else {
    console.log('Old grid template not found.');
}

// 2. CSS Replacement
const oldCSSRegex = /\.modern-appointment-card\s*\{[\s\S]*?\.action-btns\s*\{/;
const horizontalCSS = `
...
if (oldCSSRegex.test(content)) {
    content = content.replace(oldCSSRegex, horizontalCSS);
    console.log('CSS replaced.');
} else {
    console.log('Old CSS section not found.');
}

fs.writeFileSync(targetPath, content, 'utf8');
console.log('File restored and updated.');
