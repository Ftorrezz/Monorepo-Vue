      ...
      <div class="services-header">
        <div class="services-container">
          <div class="row q-col-gutter-md">
            ...
            <div class="col-8">
              <!-- Reemplaza la sección del q-select actual -->
              <div class="service-selector-modern">
                ...
                >
                  <template v-slot:option="{ itemProps, opt, selected }">
                    <q-item v-bind="itemProps" class="modern-select-item">
                      <q-item-section avatar>
                        <q-avatar :color="opt.color" text-color="white">
                          <q-icon :name="opt.icon" />
                        </q-avatar>
                      </q-item-section>
                      <q-item-section>
                        <q-item-label class="modern-select-label">{{ opt.name }}</q-item-label>
                        <q-item-label caption>
                          <div class="row items-center q-gutter-x-sm">
                            <div class="flex items-center">
                              <q-icon name="schedule" size="16px" />
                              <span class="q-ml-xs">{{ opt.duration }}min</span>
                            </div>
                            <q-separator vertical inset />
                            <div class="flex items-center">
                              <q-icon name="attach_money" size="16px" />
                              <span class="q-ml-xs">${{ opt.price }}</span>
                            </div>
                          </div>
                        </q-item-label>
                      </q-item-section>
                      <q-item-section side>
                        <q-icon name="check" color="primary" v-if="selected" />
                      </q-item-section>
                    </q-item>
                  </template>

                  <template v-slot:selected>
                    <div v-if="selectedService" class="row items-center q-gutter-x-md full-width modern-selected">
                      <q-avatar :color="selectedService.color" text-color="white">
                        <q-icon :name="selectedService.icon" />
                      </q-avatar>
                      <div class="column">
                        <div class="text-subtitle1 text-weight-bold">{{ selectedService.name }}</div>
...
/* Mejora el menú desplegable */
.modern-select :deep(.q-menu) {
  border-radius: 16px;
