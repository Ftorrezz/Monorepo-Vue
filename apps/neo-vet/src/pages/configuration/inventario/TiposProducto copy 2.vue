...
import { TipoProductoService, TipoProducto } from 'src/services/tipo-producto.service';
...
const cargarTipos = async () => {
  cargando.value = true;
  try {
    tipos.value = await TipoProductoService.getAll();
  } catch (error) {
...
const guardarTipo = async () => {
  ...
  try {
    if (tipoEditando.value) {
      await TipoProductoService.update(tipoEditando.value.id, tipoTemporal.value);
      $q.notify({ type: 'positive', message: 'Tipo actualizado correctamente' });
    } else {
      await TipoProductoService.create(tipoTemporal.value as Omit<TipoProducto, 'id'>);
...
const toggleEstado = async (tipo: TipoProducto) => {
  ...
  $q.dialog({
    ...
    try {
      await TipoProductoService.update(tipo.id, { activo: !tipo.activo });
