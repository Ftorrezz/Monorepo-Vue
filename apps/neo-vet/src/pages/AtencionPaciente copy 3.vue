...
export default {
  ...
  setup() {
    ...
    const serviciosDinamicosParaLista = computed(() => {
      const lista = Object.values(esquemasActivos.value).map(esquema => ({
        id: esquema.id,
      ...
      console.log('🎯 serviciosDinamicosParaLista computed:', lista)
      return lista
    })
    ...
    onMounted(async () => {
      try {
        ...
        console.log('📋 Servicios dinámicos cargados:', serviciosDinamicos)
        ...
        for (const srv of serviciosDinamicos.filter(s => s.activo === 'S')) {
          ...
          console.log(`📦 Esquema cargado para ${srv.nombre}:`, esquema)
          if (esquema && esquema.identificador) {
            esquemasActivos.value[esquema.identificador] = esquema
          }
        }
        console.log('✅ Esquemas activos finales:', esquemasActivos.value)
      } catch (error) {
